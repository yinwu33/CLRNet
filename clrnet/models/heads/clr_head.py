...
class CLRHead(nn.Module):
    ...
    def pool_prior_features(self, batch_features, num_priors, prior_xs):
        ...
        Args:
            ...
            num_priors (int): Number of priors, 192
            prior_xs (Tensor): X coordinates of priors on feature map, shape: (B, 192, 36)
        """

        batch_size = batch_features.shape[0]

        prior_xs = prior_xs.view(batch_size, num_priors, -1, 1)  # (B, 192, 36, 1)
        prior_ys = self.prior_feat_ys.repeat(batch_size * num_priors).view(
            batch_size, num_priors, -1, 1
        )  # (B, 192, 36, 1)

        prior_xs = prior_xs * 2.0 - 1.0  # normalize to [-1, 1]
        prior_ys = prior_ys * 2.0 - 1.0  # normalize to [-1, 1]
    ...
    def forward(self, x, **kwargs):
        ...
        for stage in range(self.refine_layers):
            num_priors = priors_on_featmap.shape[1]  # 192
            prior_xs = torch.flip(
                priors_on_featmap, dims=[2]
            )  # TODO: why [B, 192, 36] -> [B, 192, 36]
            ...
            )  # ! (B*192, 64), we need fc_features in feat space

            cls_features = fc_features.clone()
            reg_features = fc_features.clone()

            fc_features_list.append(
                fc_features.reshape(batch_size, num_priors, -1).clone()
            )
            ...
            predictions = priors.clone()  # [B, 192, 78]
            predictions[:, :, :2] = cls_logits  # 0: negative, 1: positive
        ...
        return predictions_lists[-1], fc_features_list[-1]

    def predictions_to_pred(self, predictions):
        """
        Convert predictions to internal Lane structure for evaluation.
        predictions: [num_keep, 78]
        """
        self.prior_ys = self.prior_ys.to(predictions.device)
        self.prior_ys = self.prior_ys.double()
        lanes = []
        softmax = nn.Softmax(dim=0)
        keep = [1 for _ in range(predictions.shape[0])]
        for i, lane in enumerate(predictions):
            logits = lane[:2]  # [negative_prob, positive_prob]
            lane_prob = softmax(logits)[1].item()
    ...
    def get_lanes(self, output, as_lanes=True):
        ...
        decoded = []
        lanes_feat_list = []
        for predictions, fc_features in zip(
            *output
        ):  # predictions: [192, 78], fc_features: [192, 64]
            # filter out the conf lower than conf threshold
            threshold = self.cfg.test_parameters.conf_threshold  # 0.4
            scores = softmax(predictions[:, :2])[:, 1]
            keep_inds = scores >= threshold
            predictions = predictions[keep_inds]
            fc_features = fc_features[keep_inds]  # ! mycode
            scores = scores[keep_inds]

            if predictions.shape[0] == 0:
                decoded.append([])
                lanes_feat_list.append([])  # ! mycode
                continue
            ...
            fc_features = fc_features[keep]  # ! mycode

            if predictions.shape[0] == 0:
                decoded.append([])
                lanes_feat_list.append([])
                continue

            predictions[:, 5] = torch.round(predictions[:, 5] * self.n_strips)
            if as_lanes:
                pred, keep = self.predictions_to_pred(predictions)
                keep = torch.tensor(keep).bool().to(predictions.device)
                fc_features = fc_features[keep]  # ! mycode
            else:
                pred = predictions

            assert len(pred) == len(fc_features)  # ! mycode

            decoded.append(pred)
            lanes_feat_list.append(fc_features)  # ! mycode

        assert len(decoded) == len(lanes_feat_list)

        return decoded, lanes_feat_list

...
class LLAMAS(BaseDataset):
    def __init__(
        ...
        data_root,
        split="train",
        ori_img_size=(717, 1276),
        sample_y=range(717, 300, -20),
        processes=None,
        cfg=None,
    ):
        self.data_root = data_root
        super().__init__(data_root, split, ori_img_size, sample_y, processes, cfg)
    ...
    def get_img_name(self, json_path):
        base_name = (json_path.split("/")[-1]).replace(".json", "_color_rect.png")
        return base_name
    ...
    def load_annotations(self):
        ...
        for json_path in tqdm(json_paths):
            ...
            lanes = [
                [(x, y) for x, y in zip(lane, range(self.ori_img_h)) if x >= 0]
                for lane in lanes
            ]
    ...
    def get_prediction_string(self, pred):
        ...
        for lane in pred:
            xs = lane(ys)
            valid_mask = (xs >= 0) & (xs < 1)
            xs = xs * (self.ori_img_w - 1)  # ! remove cfg
            lane_xs = xs[valid_mask]
            lane_ys = ys[valid_mask] * (self.ori_img_h - 1)  # ! remove cfg
    ...
    def evaluate(self, predictions, output_basedir):
        ...
        result = llamas_metric.eval_predictions(
            ...
            unofficial=False,
            model_img_size=(self.cfg.ori_img_h, self.cfg.ori_img_w),
            cfg=self.cfg,
        )
